@php
    $dashSortUrl = function(string $col) use ($sort, $dir, $statsFrom, $statsTo): string {
        $newDir = ($sort === $col && $dir === 'desc') ? 'asc' : 'desc';
        return route('admin.dashboard', array_merge(request()->except('sort','dir','page'), [
            'sort'       => $col,
            'dir'        => $newDir,
    ...
    <table class="min-w-full divide-y divide-gray-100">
        <thead class="bg-gray-50">
            <tr>
                <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">
                    <a href="{{ $dashSortUrl('reference') }}" class="dash-sort flex items-center gap-1 hover:text-gray-700 whitespace-nowrap">
                ...
                <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">
                    <a href="{{ $dashSortUrl('check_in') }}" class="dash-sort flex items-center gap-1 hover:text-gray-700 whitespace-nowrap">
                        1er Check-in {!! $sortIcon('check_in') !!}
                    </a>
                </th>
                <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">
                    <a href="{{ $dashSortUrl('check_out') }}" class="dash-sort flex items-center gap-1 hover:text-gray-700 whitespace-nowrap">
                        Dernier Check-out {!! $sortIcon('check_out') !!}
                    </a>
                </th>
                ...
                <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">
                    <a href="{{ $dashSortUrl('total_price') }}" class="dash-sort flex items-center gap-1 hover:text-gray-700 whitespace-nowrap">
                        Total {!! $sortIcon('total_price') !!}
                    </a>
                </th>
                <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Statut</th>
                <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">
                    <a href="{{ $dashSortUrl('created_at') }}" class="dash-sort flex items-center gap-1 hover:text-gray-700 whitespace-nowrap">
        ...
        <tbody class="divide-y divide-gray-100 bg-white">
            ...
            <tr class="hover:bg-amber-50/40 transition-colors">
                <td class="px-4 py-3 text-sm font-mono font-medium text-gray-900">{{ $res->reference }}</td>
                <td class="px-4 py-3">
                    <p class="text-sm font-medium text-gray-900">{{ $res->agency_name }}</p>
                ...
                <td class="px-4 py-3 text-xs text-gray-400 whitespace-nowrap">{{ $res->created_at->format('d/m/Y H:i') }}</td>
                <td class="px-4 py-3">
                    <a href="{{ route('admin.reservations.show', $res) }}"
                       class="inline-flex items-center gap-1 text-xs font-medium text-amber-600 hover:text-amber-700 bg-amber-50 hover:bg-amber-100 px-2 py-1 rounded transition-colors">
                        Voir
                    </a>
                </td>
            </tr>
...
{{-- Pagination --}}
@if($recent->hasPages())
<div class="px-6 py-4 border-t border-gray-100 dash-pagination">
    {{ $recent->links() }}
</div>
@endif
