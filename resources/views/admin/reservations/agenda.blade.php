...
@section('title', 'Agenda des arrivées')
@section('page-title', 'Agenda des arrivées')

@section('header-actions')
    <div class="flex items-center gap-2"> <a href="{{ route('admin.reservations.agenda-depart', array_merge(['month' => $month, 'hotel_id' => $hotelId ?? ''], ['statuses' => $selectedStatuses])) }}"
           class="bg-white border border-red-200 hover:bg-red-50 text-red-700 text-sm font-medium px-4 py-2 rounded-lg flex items-center gap-1.5"> Départs
...
// Construire la grille du mois (remplir depuis lundi)
...
<div class="space-y-4"> {{--  Barre de navigation  --}}
    <div class="flex flex-wrap items-center gap-3"> {{-- Filtre hôtel --}}
        <form method="GET" action="{{ route('admin.reservations.agenda') }}" id="hotelForm" class="flex items-center gap-2"> <input type="hidden" name="month" value="{{ $month }}"> @foreach($selectedStatuses as $s)
            <input type="hidden" name="statuses[]" value="{{ $s }}"> @endforeach
            <select name="hotel_id" onchange="this.form.submit()"
                class="border border-gray-300 rounded-lg px-3 py-2 text-sm bg-white focus:ring-2 focus:ring-amber-400 focus:outline-none"> <option value="">Tous les hôtels</option> @foreach($hotels as $h)
                <option value="{{ $h->id }}" {{ $hotelId == $h->id ? 'selected' : '' }}>{{ $h->name }}</option> @endforeach
            </select> </form> {{-- Navigation mois --}}
        <div class="flex items-center gap-1 bg-white border border-gray-200 rounded-xl overflow-hidden"> <a href="{{ route('admin.reservations.agenda', array_merge(['month' => $prevMonth, 'hotel_id' => $hotelId], ['statuses' => $selectedStatuses])) }}"
            ...
            </span> <a href="{{ route('admin.reservations.agenda', array_merge(['month' => $nextMonth, 'hotel_id' => $hotelId], ['statuses' => $selectedStatuses])) }}"
        ...
        <a href="{{ route('admin.reservations.agenda', array_merge(['month' => now()->format('Y-m'), 'hotel_id' => $hotelId], ['statuses' => $selectedStatuses])) }}"
           class="text-sm text-amber-600 hover:text-amber-800 border border-amber-200 bg-amber-50 px-3 py-2 rounded-lg transition-colors"> Aujourd'hui
        </a> {{-- Stats --}}
        <div class="ml-auto flex items-center gap-3 text-xs text-gray-500"> @if($todayArrivals > 0)
            <span class="flex items-center gap-1.5 bg-amber-50 border border-amber-200 text-amber-700 font-semibold px-3 py-1.5 rounded-lg"> <span class="w-2 h-2 bg-amber-400 rounded-full animate-pulse"></span> {{ $todayArrivals }} arrivée(s) aujourd'hui
            </span> @endif
            <span class="bg-gray-50 border border-gray-200 px-3 py-1.5 rounded-lg"> {{ $totalArrivals }} arrivée(s) ce mois
            </span> </div> </div> {{--  Filtre statuts (boutons toggle)  --}}
    <form method="GET" action="{{ route('admin.reservations.agenda') }}" id="statusForm"> <input type="hidden" name="month" value="{{ $month }}"> <input type="hidden" name="hotel_id" value="{{ $hotelId }}"> <div class="flex flex-wrap gap-2 items-center"> <span class="text-xs text-gray-500 font-medium">Filtrer :</span> @foreach($statusColors as $status => $colors)
    ...
    <div class="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden"> {{-- Jours de la semaine --}}
        ...
        <div class="grid grid-cols-7"> @foreach($calDays as $idx => $day)
            ...
            <div class="min-h-[110px] p-2 border-b border-gray-100 flex flex-col gap-1
                ...
                </div> {{-- Groupes arrivants --}}
                @foreach($arrivals->take(3) as $res)
                @php
                    $sc = $statusColors[$res->status] ?? $statusColors['draft'];
                    // Séjour correspondant à cette date
                    $sejoursOnDate = $res->rooms->filter(
                        fn($r) => $r->check_in && $r->check_in->format('Y-m-d') === $dateKey
                    );
                    $sejourCheckOut = $sejoursOnDate->first()?->check_out;
                    $sejourNights   = $sejourCheckOut
                        ? \Carbon\Carbon::parse($dateKey)->diffInDays($sejourCheckOut)
                        : null;
                    // Numéro du séjour si multi-séjour
                    $allSejourDates = $res->rooms->filter(fn($r) => $r->check_in !== null)
                        ->map(fn($r) => $r->check_in->format('Y-m-d'))->unique()->sort()->values();
                ...
                @if($count > 3)
                @php
                    $popupData = $arrivals->map(function($res) use ($statusColors, $dateKey) {
                        $sc = $statusColors[$res->status] ?? $statusColors['draft'];
                        $allDates  = $res->rooms->filter(fn($r) => $r->check_in !== null)
                            ->map(fn($r) => $r->check_in->format('Y-m-d'))->unique()->sort()->values();
                        $sejourIdx = $allDates->search($dateKey);
                        $isMulti   = $allDates->count() > 1;
                        $sejoursOnDate  = $res->rooms->filter(fn($r) => $r->check_in && $r->check_in->format('Y-m-d') === $dateKey);
                        $sejourNights   = $sejoursOnDate->first()?->check_out
                            ? \Carbon\Carbon::parse($dateKey)->diffInDays($sejoursOnDate->first()->check_out)
                            : null;
                        return [
                ...
                <button type="button"
                        ...
                        class="text-[10px] text-amber-600 hover:text-amber-800 font-semibold text-center w-full cursor-pointer hover:underline">
                    +{{ $count - 3 }} autres
                </button>
                @endif
            </div> @endforeach
        </div> </div> {{--  Liste du jour (si aujourd'hui a des arrivées)  --}}
    ...
    <div class="bg-amber-50 border border-amber-200 rounded-2xl p-4"> <h3 class="text-sm font-bold text-amber-800 mb-3 flex items-center gap-2"> <span class="w-2 h-2 bg-amber-400 rounded-full animate-pulse"></span> Arrivées aujourd'hui ({{ $todayList->count() }})
        </h3> <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2"> @foreach($todayList as $res)
            @php
                $sc = $statusColors[$res->status] ?? $statusColors['draft'];
                // Séjour du jour
                $todayRooms = $res->rooms->filter(fn($r) => $r->check_in && $r->check_in->isToday());
                $todaySejour = $todayRooms->first();
                $todayNights = $todaySejour?->check_out
                    ? now()->diffInDays($todaySejour->check_out)
                    : $res->nights;
                // Numéro du séjour si multi-séjour
                $allDates2 = $res->rooms->filter(fn($r) => $r->check_in !== null)
                    ->map(fn($r) => $r->check_in->format('Y-m-d'))->unique()->sort()->values();
                $sejourIdx2 = $allDates2->search(now()->format('Y-m-d'));
                $isMulti2   = $allDates2->count() > 1;
            @endphp
            <a href="{{ route('admin.reservations.show', $res) }}"
               class="flex items-center gap-3 bg-white border border-amber-100 rounded-xl px-4 py-3 hover:border-amber-300 hover:shadow-sm transition-all"> <span class="w-3 h-3 rounded-full shrink-0 {{ $sc['dot'] }}"></span> <div class="min-w-0 flex-1"> <div class="text-sm font-semibold text-gray-800 truncate flex items-center gap-1.5"> {{ $res->agency_name ?: $res->contact_name }}
                        @if($isMulti2)
                        <span class="text-[10px] font-bold bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded-full shrink-0">Séj. {{ $sejourIdx2 + 1 }}</span> @endif
...
function openDayPopup(dateKey, btn) {
    ...
    const title = parseInt(d) + ' ' + MONTHS_FR[parseInt(m) - 1] + ' ' + y
                + ' \u2014 ' + data.length + ' arriv\u00e9e' + (data.length > 1 ? 's' : '');

    document.getElementById('day-popup-title').textContent = title;
    document.getElementById('day-popup-list').innerHTML = data.map(r => `
        <a href="${r.url}" class="flex items-center gap-3 border border-gray-100 rounded-xl px-4 py-3 hover:border-amber-200 hover:bg-amber-50 transition-all">
            ...
            <div class="flex-1 min-w-0">
                <div class="text-sm font-semibold text-gray-800 truncate flex items-center gap-1.5">
                    ${esc(r.name)}
                    ${r.sejour ? '<span class="text-[10px] font-bold bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded-full shrink-0">S\u00e9j.\u00a0' + r.sejour + '</span>' : ''}
                </div>
